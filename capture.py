...
# an initial bandpass
	...
	default(gaincal)
	print 'reached here...'
	print str(msfilename)+'.K1'+mycalsuffix,str(msfilename)+'.AP.G0'+mycalsuffix,(','.join(mybpcals)),flagspw,ref_ant
	gaincal(vis=msfilename, caltable=str(msfilename)+'.AP.G0'+mycalsuffix, append=True, field=str(','.join(mybpcals)), spw =flagspw, solint = 'int', refant = ref_ant, minsnr = 2.0, solmode = 'L1R', gaintype = 'G', calmode = 'ap', gaintable = [str(msfilename)+'.K1'+mycalsuffix], interp = ['nearest,nearestflag', 'nearest,nearestflag' ], parang = True)
	print 'wrote table :',str(msfilename)+'.AP.G0'+mycalsuffix,str(msfilename)+'.K1'+mycalsuffix
...
#For the target:
	...
	logging.info("Finished initial calibration.")
        ...
        flagsummary(msfilename)
	############# MSJ
	plot_files = my_dig_plot(msfilename,myfields,myampcals,mypcals,mytargets,mycalsuffix,bptable)
	############# MSJ
#############################################################################3
...
if redocal == True:
	...
	for i in range(0,len(myampcals)):
		...
		print 'Setjy Done for source ',myampcals[i]
# Delay calibration  using the first flux calibrator in the list - should depend on which is less flagged
	if os.path.isdir(str(msfilename)+'.K1'+mycalsuffix) == True:
		os.system('rm -rf '+str(msfilename)+'.K1'+mycalsuffix)
	gaincal(vis=msfilename, caltable=str(msfilename)+'.K1'+mycalsuffix, spw =flagspw, field=myampcals[0], 
...
# an initial bandpass
	...
	default(bandpass)
	bptable=str(msfilename)+'.B1'+mycalsuffix
	bandpass(vis=msfilename, caltable=bptable, spw =flagspw, field=str(','.join(mybpcals)), solint='inf', refant=ref_ant, solnorm = True, minsnr=2.0, fillgaps=8, parang = True, gaintable=[str(msfilename)+'.K1'+mycalsuffix,str(msfilename)+'.AP.G0'+mycalsuffix], interp=['nearest,nearestflag','nearest,nearestflag'])
